...
void GoalNet::ComputeInternalForce()
{
	...
	for (int i = 0; i<this->SpringNum(); i++){
		...
		CSpring* s = &m_Springs[i];
		int start = s->GetSpringStartID();
		int end = s->GetSpringEndID();

		Vector3d p1 = m_Particles[start].GetPosition();
		Vector3d p2 = m_Particles[end].GetPosition();
		Vector3d v1 = m_Particles[start].GetVelocity();
		Vector3d v2 = m_Particles[end].GetVelocity();
		m_Particles[start].AddForce(ComputeSpringForce(p1, p2, m_dSpringCoefStruct, s->GetSpringRestLength()));
		m_Particles[end].AddForce(-ComputeSpringForce(p1, p2, m_dSpringCoefStruct, s->GetSpringRestLength()));
		m_Particles[start].AddForce(-ComputeDamperForce(p1, p2, v1, v2, m_dDamperCoefStruct));
		m_Particles[end].AddForce(ComputeDamperForce(p1, p2, v1, v2, m_dDamperCoefStruct));
	}
...
// TO DO 1
	//add horizontal spring
		for (int y = 0 ; y < m_NumAtHeight ;  y++){
		    for (int x = 0 ;  x < m_NumAtWidth - 1 ;  x++){
				this->addStructSpring(GetParticleID(x, y, 0), GetParticleID(x + 1, y, 0));
				this->addStructSpring(GetParticleID(x, y, m_NumAtLength - 1), GetParticleID(x + 1, y, m_NumAtLength - 1));
			}
			for (int z = 0; z < m_NumAtLength - 1; z++){
				this->addStructSpring(GetParticleID(0, y, z), GetParticleID(0, y, z+1));
				this->addStructSpring(GetParticleID(m_NumAtWidth - 1, y, z), GetParticleID(m_NumAtWidth - 1, y, z + 1));
			}
		}
	//add verizonal spring
		for (int x = 0; x < m_NumAtWidth; x++){
			for (int y = 0; y < m_NumAtHeight-1; y++){
				this->addStructSpring(GetParticleID(x, y, 0), GetParticleID(x, y + 1, 0));
				this->addStructSpring(GetParticleID(x, y, m_NumAtLength - 1), GetParticleID(x, y + 1, m_NumAtLength - 1));
			}
		}
		for (int z = 0; z < m_NumAtLength; z++){
			for (int y = 0; y < m_NumAtHeight - 1; y++){
				this->addStructSpring(GetParticleID(0, y, z), GetParticleID(0, y+1 , z));
				this->addStructSpring(GetParticleID(m_NumAtWidth - 1, y, z), GetParticleID(m_NumAtWidth - 1, y+1 , z ));
			}
		}
	//add bottom spring
		for (int x = 0; x < m_NumAtWidth-1; x++){
			for (int z = 0; z < m_NumAtLength - 1; z++){
				this->addStructSpring(GetParticleID(x, 0, z), GetParticleID(x + 1, 0, z));
				this->addStructSpring(GetParticleID(x, 0, z), GetParticleID(x, 0, z + 1));
			}
		}
	...
	cout << (m_Particles[0].GetPosition() - m_Particles[10].GetPosition()).Length() << endl;
}
...
{
	Vector3d f = (a_crPos1 - a_crPos2) /((a_crPos1 - a_crPos2).Length()) ;
	double coef = -a_cdSpringCoef*((a_crPos1 - a_crPos2).Length() - a_cdRestLength);
	return coef * f;
}
...
{
    //TO DO 4
	Vector3d f =(((a_crVel1 - a_crVel2).DotProduct(a_crPos1 - a_crPos2)) / (a_crPos1 - a_crPos2).Length()) *((a_crPos1 - a_crPos2) / (a_crPos1 - a_crPos2).Length());
	return a_cdDamperCoef * f;
}
